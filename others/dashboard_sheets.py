...
tabs = st.tabs(["📊 Orders", "🛰️ Spacecraft", "🌎 Geography", "📋 Details"])
...
with tabs[0]:
    # st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; color: var(--primary-color); font-size: 15px;'>Order Trends</h3>", unsafe_allow_html=True)
...
with tabs[1]:
    # st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; color: var(--primary-color); font-size: 15px;'>Spacecraft Usage Analysis</h3>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div class='compact-metric'>
        <div class='metric-label'>Total Spacecraft Usage</div>
        <div class='metric-value'>{total_minutes:,.2f} minutes</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        html_table += """<tr style='background-color: #E3F2FD;'>
            <th style='padding: 6px; text-align: center;'>Icon</th>
            <th style='padding: 6px; text-align: left;'>Spacecraft</th>
        ...
        for i, row in spacecraft_counts.iterrows():
            ...
            html_table += f"""<tr style='background-color: {bg_color};'>
                ...
                <td style='padding: 6px; text-align: right;'>{row['Orders']:,}</td>
                <td style='padding: 6px; text-align: right;'>{row['Percentage']}%</td>
                <td style='padding: 6px; text-align: right;'>{row['Minutes']:,.2f}</td>
            </tr>"""
...
with tabs[2]:
    ...
    with col2:
        st.markdown("<h4 style='text-align: center; font-size: 14px;'>List of Countries</h4>", unsafe_allow_html=True)
        ...
        country_stats.columns = ['Country', 'Total Area (sqkm)', 'Avg Area (sqkm)', 'Orders']
        ...
        html_container = """<div style='height: 400px; overflow-y: auto; overflow-x: hidden; padding: 0; margin: 0; border: 1px solid #e0e0e0; border-radius: 5px;'>"""
        ...
        html_table += """<tr style='background-color: #E3F2FD; position: sticky; top: 0; z-index: 10;'>
            <th style='padding: 6px; text-align: left; width: 30%;'>Country</th>
            <th style='padding: 6px; text-align: right; width: 15%;'>Total Area (sqkm)</th>
            <th style='padding: 6px; text-align: right; width: 15%;'>Avg Area (sqkm)</th>
            <th style='padding: 6px; text-align: right; width: 15%;'>No of Orders</th>
        </tr>"""
        
        for i, row in country_stats.iterrows():
            bg_color = '#F5F7FA' if i % 2 == 0 else 'white'
            html_table += f"""<tr style='background-color: {bg_color}; border-bottom: 1px solid #e0e0e0;'>
                <td style='padding: 6px; text-align: left; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{row['Country']}</td>
                <td style='padding: 6px; text-align: right; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{row['Total Area (sqkm)']:,.2f}</td>
                <td style='padding: 6px; text-align: right; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{row['Avg Area (sqkm)']:,.2f}</td>
                <td style='padding: 6px; text-align: right; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>{row['Orders']}</td>
            </tr>"""
...
with tabs[3]:
    # st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.markdown("<h3 style='text-align: center; color: var(--primary-color); font-size: 15px;'>Order Details Analysis</h3>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown(f"""
        <div class='compact-metric'>
            <div class='metric-label'>Average Area per Order</div>
            <div class='metric-value'>{avg_sqkm:.2f} sqkm</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown(f"""
        <div class='compact-metric'>
            <div class='metric-label'>Average Time to Complete</div>
            <div class='metric-value'>{avg_time_to_complete:.2f} minutes</div>
        </div>
        """, unsafe_allow_html=True)
    
    # Create tabs for different detail views
    detail_tabs = st.tabs(["📦 Product Types", "📋 Order Types", "⏱️ Responsiveness"])
    ...
    with detail_tabs[0]:
        ...
        with pcol2:
            ...
            html_table = """<table style='width:100%; border-collapse: collapse;'>""" 
            html_table += """<tr style='background-color: #E3F2FD;'>
                <th style='padding: 6px; text-align: left;'>Product Type</th>
            ...
            for i, row in product_counts.iterrows():
                bg_color = '#F5F7FA' if i % 2 == 0 else 'white'
                html_table += f"""<tr style='background-color: {bg_color};'>
                    <td style='padding: 6px; text-align: left;'>{row['Product Type']}</td>
    ...
    with detail_tabs[1]:
        ...
        with ocol2:
            ...
            for i, row in order_type_counts.iterrows():
                bg_color = '#F5F7FA' if i % 2 == 0 else 'white'
                html_table += f"""<tr style='background-color: {bg_color};'>
                    <td style='padding: 6px; text-align: left;'>{row['Order Type']}</td>
                    <td style='padding: 6px; text-align: right;'>{row['Count']}</td>
                    <td style='padding: 6px; text-align: right;'>{row['Percentage']}%</td>
                </tr>"""
    ...
    with detail_tabs[2]:
        ...
        with rcol2:
            ...
            html_table = """<table style='width:100%; border-collapse: collapse;'>""" 
            html_table += """<tr style='background-color: #E3F2FD;'>
                <th style='padding: 6px; text-align: left;'>Responsiveness</th>
                <th style='padding: 6px; text-align: right;'>Count</th>
                <th style='padding: 6px; text-align: right;'>%</th>
            </tr>"""
            ...
            for i, row in responsiveness_counts.iterrows():
                bg_color = '#F5F7FA' if i % 2 == 0 else 'white'
                html_table += f"""<tr style='background-color: {bg_color};'>
                    <td style='padding: 6px; text-align: left;'>{row['Responsiveness']}</td>
                    <td style='padding: 6px; text-align: right;'>{row['Count']}</td>
                    <td style='padding: 6px; text-align: right;'>{row['Percentage']}%</td>
                </tr>"""
...
st.dataframe(
    ...
    column_config={
        ...
        "Archive/Tasking": st.column_config.TextColumn(
            "Archive/Tasking",
            help="Archive or Tasking type",
            width="small"
        ),
        ...
        "Seconds": st.column_config.NumberColumn(
            "Seconds",
            help="Time to complete in seconds",
            format="%.0f",
            width="small"
        ),
